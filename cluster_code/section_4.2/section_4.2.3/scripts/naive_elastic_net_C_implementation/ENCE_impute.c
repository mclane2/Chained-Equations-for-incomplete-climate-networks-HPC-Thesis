 ...
 * 
 * Author: M. Lane
 * Version: 1.1 (added write betas to file)
 * Date: 2026-05-29
 */
 ...
 *
 */
static void column_impute(const double *df_old, double *target_out, int n, int p, int col, const int *missing_col, const int *fold_col,
                          ...
                          double *lambda_io, double *alpha_io)
{
    ...
    if (n_miss == 0) return; // no missing entries
    ...
    if (lambda < 0.0) {
        ...
        /* Malloc error check */
        if (!cvm || !fold_obs || !train_id || !val_id || !xt || !xv || !yt || !yv) {
        ...
        for (int f = 1; f <= nfolds; ++f) {
            ...
            /* Try all combinations of lambda and alpha */
            for (int l = 0; l < n_lambda; ++l) {
                for (int a = 0; a < n_alpha; ++a) {
                    double intercept;

                    elastic_net_fit(xt, yt, n_train, p_cov, lambdas[l], alphas[a], thresh, maxit, beta, &intercept);

                    /* Test on held-out validation data */
                    cvm[a + (size_t)l * n_alpha] += predict_mse(xv, n_val, p_cov, beta, intercept, yv) / nfolds;
                }
            }
        }
    ...
    double intercept;
    elastic_net_fit(x_obs, y_obs, n_obs, p_cov, lambda, alpha, thresh, maxit, beta, &intercept);
    ...
    *alpha_io  = alpha;
    ...
    #pragma omp critical
    {
        FILE *fp = fopen("betas_naive.csv", "a");
        if (fp) {
            for (int jc = 0; jc < p_cov; ++jc) {
 ...
 */
void ence_impute_cycle(const double *df_old, double *df_new, const int *missing_idx, const int *folds, int n, int p, const double *lambdas, int n_lambda,
                       const double *alphas,  int n_alpha, int nfolds, double thresh, int maxit, int nthreads, double *lambda_io, double *alpha_io)
{
    ...
    FILE *fp = fopen("betas_naive.csv", "w");
    if (fp) {
    ...
    for (int col = 0; col < p; ++col) {
        ...
        column_impute(df_old, df_new + (size_t)col * n, n, p, col, missing_idx + (size_t)col * n, folds + (size_t)col * n,
                      lambdas, n_lambda, alphas, n_alpha, nfolds, thresh, maxit, &lambda_io[col], &alpha_io[col]);
    }
}