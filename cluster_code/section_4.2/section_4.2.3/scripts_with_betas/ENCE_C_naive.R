...
## Calls naive version of C implementation instead of glmnet ##
...
ENCE_C_naive <- function(df, response = "y",
                 hyp_cycles = 2,
                 max_cycles = 16,
  ...
  while((i <= max_cycles) & ((old_rmse/new_rmse > tol) | (i == 1))){
    ...
    imputed_df <- naive_ence_impute_cycle(df, missing_idx, folds, ls, as,
    ...
    if (file.exists("betas_naive.csv")) {
      cyc_betas <- read.csv("betas_naive.csv")
      cyc_betas$cycle <- i
