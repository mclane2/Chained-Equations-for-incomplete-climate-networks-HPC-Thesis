...
## Calls warmstart version of C implementation instead of glmnet ##
...
## - Added beta history tracker

ENCE_C_warmstart <- function(df, response = "y",
                 hyp_cycles = 2,
                 max_cycles = 16,
  ...
  while((i <= max_cycles) & ((old_rmse/new_rmse > tol) | (i == 1))){
    ...
    imputed_df <- warmstart_ence_impute_cycle(df, missing_idx, folds, ls, as,
    ...
    ls_history[[i]] <- ls
    as_history[[i]] <- as
    # Archive this cycle's beta snapshot
    if (file.exists("betas_warmstart.csv")) {
      cyc_betas <- read.csv("betas_warmstart.csv")
      cyc_betas$cycle <- i
      beta_history[[i]] <- cyc_betas
    }
  ...
  attr(df, "beta_history") <- do.call(rbind, beta_history)
