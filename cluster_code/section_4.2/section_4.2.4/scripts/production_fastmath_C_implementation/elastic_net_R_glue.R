...
dyn.load("scripts/production_fastmath_C_implementation/fastmath_ENCE_impute.so")

# One synchronous imputation cycle, parallel across stations in C.
# Returns list(df, lambda, alpha).
fastmath_ence_impute_cycle <- function(df_old, missing_idx, folds, lambda_io, alpha_io, 
  ...
  .Call("fastmath_ence_impute_cycle_R", df_mat, miss, fld,
        as.double(lambdas), as.double(alphas),
        as.integer(nfolds), as.double(thresh), as.integer(maxit),
        as.integer(nthreads), lam, alp, PACKAGE = "fastmath_ENCE_impute")
}