...
dyn.load("scripts/unroll_C_implementation/unroll_ENCE_impute.so")

# One synchronous imputation cycle, parallel across stations in C.
# Returns list(df, lambda, alpha).
unroll_ence_impute_cycle <- function(df_old, missing_idx, folds, lambda_io, alpha_io, 
  ...
  .Call("unroll_ence_impute_cycle_R", df_mat, miss, fld,
        as.double(lambdas), as.double(alphas),
        as.integer(nfolds), as.double(thresh), as.integer(maxit),
        as.integer(nthreads), lam, alp, PACKAGE = "unroll_ENCE_impute")
}