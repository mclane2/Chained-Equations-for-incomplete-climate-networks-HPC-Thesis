...
## Calls guided version of C implementation instead of glmnet ##
...
ENCE_C_guided <- function(df, response = "y",
                 hyp_cycles = 2,
                 max_cycles = 16,
  ...
  while((i <= max_cycles) & ((old_rmse/new_rmse > tol) | (i == 1))){
    ...
    imputed_df <- guided_ence_impute_cycle(df, missing_idx, folds, ls, as,
