...
impute_MLR <- function(data, newdata, nn = 5, response = "y",
  ...
  Wfill_wide <- df_filled %>% dplyr::select(all_of(c(t_name, "stno", response))) %>%
    pivot_wider(names_from = "stno", values_from = all_of(response)) %>%
    arrange(.data[[t_name]])
    Wfill <- Wfill_wide %>% dplyr::select(-all_of(t_name)) %>% as.matrix()
  ...
  for(target in unique(df$stno[missing])){
    ...
    nn_use <- min(nn, length(dists))
    neigh <- match(as.character(other_sdf$stno[order(dists)][1:nn_use]), colnames(Wfill))
    ...
    if (anyNA(model)) warning(sprintf("station %d: Error: no coefficients", target))
  ...
  new_df[response] <- reverse_transformation(new_df[response])
