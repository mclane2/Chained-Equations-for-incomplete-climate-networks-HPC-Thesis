...
MICE_DURR_cov <- function(df, response = "y", m = 4, 
                      max_impute_cycles = 5, 
                      max_EM_cycles = 5, 
  ...
  if(nclusters > 1){
    ...
    clusterEvalQ(cl, dyn.load("scripts/MICE_elastic_net_C_implementation_cov/MICE_impute_cov.so"))
...
MICE_get_params <- function(df, missing_idx, max_cycles = 16, 
  ...
  while((i <= max_cycles) & ((old_rmse/new_rmse > impute_tol) | (i == 1))){
    ...
    imputed_df <- durr_impute_cycle_cov(df, missing_idx, folds, ls, as, boot, noise,
