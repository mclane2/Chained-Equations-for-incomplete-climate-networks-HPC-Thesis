                          ...
                          double *lambda_io, double *alpha_io)
{
    ...
    /* Malloc error check*/
    if (!obs_rows || !miss_rows || !x_obs || !x_miss || !y_obs || !beta || !C || !has_row) {
        free(obs_rows); free(miss_rows); free(x_obs); free(x_miss); free(y_obs); free(beta); free(C); free(has_row);
        return 1;
    }
    ...
    if (lambda < 0.0) {
        ...
        if (!cvm || !fold_obs || !train_id || !val_id || !xt || !xv || !yt || !yv || !beta_path || !int_path ) {
            free(cvm); free(fold_obs); free(train_id); free(val_id); free(xt); free(xv); free(yt); free(yv);free(obs_rows); free(miss_rows); free(x_obs);
