...
dyn.load("scripts/ENCE_elastic_net_C_implementation_cov/ENCE_impute_cov.so")

# One synchronous imputation cycle, parallel across stations in C.
# Returns list(df, lambda, alpha).
ence_impute_cycle_cov <- function(df_old, missing_idx, folds, lambda_io, alpha_io, 
  ...
  .Call("ence_impute_cycle_R_cov", df_mat, miss, fld,
        as.double(lambdas), as.double(alphas),
        as.integer(nfolds), as.double(thresh), as.integer(maxit),
        as.integer(nthreads), lam, alp, PACKAGE = "ENCE_impute_cov")
}