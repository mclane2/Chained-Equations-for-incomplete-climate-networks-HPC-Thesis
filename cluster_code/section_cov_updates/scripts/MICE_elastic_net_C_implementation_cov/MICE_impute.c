 ...
 * 
 * Author: M. Lane
 * Version: 4.0 (v.3 Editted for MICE DURR) (v.4 column_impute now calls elastic_net_path_cov()
 *          instead of elastic_net_path(); C and has_row allocated once per
 *          column_impute call, reused across every alpha within a fold,
 *          reset via memset whenever the underlying data changes)
 * Date: 2026-07-22 
 */
                          ...
                          double *lambda_io, double *alpha_io, const double *noise_col, double *beta_out, const int *boot_col)
{
    ...
    /* C and has_row for covariance-update coordinate descent*/
    double *C = malloc((size_t)p_cov * p_cov * sizeof(double));
    unsigned char *has_row = malloc((size_t)p_cov * sizeof(unsigned char));

    /* Malloc error check*/
    if (!miss_rows || !x_obs || !x_miss || !y_obs || !beta || !C || !has_row) {
    ...
    if (lambda < 0.0) {
        ...
        if (!cvm || !fold_obs || !train_id || !val_id || !xt || !xv || !yt || !yv || !beta_path || !int_path ) {
            free(cvm); free(fold_obs); free(train_id); free(val_id); free(xt); free(xv); free(yt); free(yv);free(miss_rows); free(x_obs); free(x_miss); free(y_obs); 
        ...
        for (int f = 1; f <= nfolds; ++f) {
            ...
            memset(has_row, 0, (size_t)p_cov * sizeof(unsigned char));

            for (int a = 0; a < n_alpha; ++a) {
                if (elastic_net_path_cov(xt, yt, n_train, p_cov, lambdas, n_lambda, alphas[a], thresh, maxit, beta_path, int_path, C, has_row) < 0) {
    ...
    double intercept;
    if (elastic_net_path_cov(x_obs, y_obs, n_obs, p_cov, &lambda, 1, alpha, thresh, maxit, beta, &intercept, C, has_row) < 0) {
        free(miss_rows); free(x_obs); free(x_miss); free(y_obs); free(beta); free(C); free(has_row);
        return 1;
    }
