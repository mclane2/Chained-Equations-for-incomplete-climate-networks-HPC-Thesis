...
## R-parallel Elastic-net Chained Equations with synchonous updates primary functions ##
...
ENCE <- function(df, response = "y",
  ...
  while((i <= max_cycles) & ((old_rmse/new_rmse > tol) | (i == 1))){
    ...
    imputed_df <- ENCE_impute_parallel(df, missing_idx, ls, as, n_cores = n_cores, ...)
    df <- imputed_df$df
    ls <- imputed_df$ls
    as <- imputed_df$as
