...
## R-parallel Elastic-net Chained Equations with synchonous updates functions ##
...
ENCE <- function(df, response = "y",
  ...
  while((i <= max_cycles) & ((old_rmse/new_rmse > tol) | (i == 1))){
    ...
    if (!is.null(truth) && !is.null(masked_idx)) {
      ...
      tmp <- pivot_longer(tmp, cols = everything(), names_to = spatial_id, values_to = response)
      tmp <- tmp %>%
      ...
      ext_rmse_history <- c(ext_rmse_history, sqrt(mean((truth[masked_idx] - imputed_y[masked_idx])^2)))
    }
  ...
  colnames(resid_mat) <- substr(names(df), nchar(spatial_id)+1,nchar(names(df)))
...
ENCE_impute_parallel <- function(df, missing_idx, ls = NA, as = NA,
  ...
  # Constructing matrix of residuals for observed values
  resid_mat <- sapply(results, function(r) r$resid)

  return(list("df" = df, "ls" = ls, "as" = as, "resid" = resid_mat))
}
...
column_impute <- function(covariates, target, missing_values, 
  ...
  # NA at every cell by default
  resid <- rep(NA_real_, length(unlist(target)))
  ...
  if (any(missing_values)){
    ...
    # Recording the residuals for fitted values for the observed data
    fitted_obs <- predict(model, x[!ymiss,,drop=F], s = lambda, alpha = alpha)
    resid[!ymiss] <- y[!ymiss] - as.numeric(fitted_obs)
  
  }
  
  return(list("target" = target, "lambda" = lambda, "alpha" = alpha, "resid" = resid))
}
