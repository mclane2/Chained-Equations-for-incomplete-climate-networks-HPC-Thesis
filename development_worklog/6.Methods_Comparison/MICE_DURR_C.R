...
MICE_DURR <- function(df, response = "y", m = 20, 
  ...
  if(nclusters > 1){
    ...
    clusterExport(cl, c("rmse", "make_folds", "durr_impute_cycle"))
    clusterEvalQ(cl, dyn.load("MICE_elastic_net_C_implementation/ENCE_impute.so"))
...
MICE_get_params <- function(df, missing_idx, max_cycles = 16, 
 ...
 # Per-station lambda/alpha vectors (NA => fit via CV this cycle)
  ...
  # Prepare bootstrapped matrices for each station for C implementation
  n <- nrow(df); p <- ncol(df)
  boot  <- matrix(0L, n, p)
  noise <- matrix(0,  n, p)
