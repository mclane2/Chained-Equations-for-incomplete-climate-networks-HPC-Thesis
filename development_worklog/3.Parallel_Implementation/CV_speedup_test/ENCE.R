...
ENCE <- function(df, response = "y", 
  ...
  if(nclusters > 1){
    ...
    clusterEvalQ(cluster, library(glmnet))   # workers need glmnet
  } else { cluster <- NULL }
  ...
  while((i <= max_cycles) & ((old_rmse/new_rmse > tol) | (i == 1))){
    ...
    imputed_df <- ENCE_impute(df, missing_idx, ls, as, cluster = cluster, ...)
    df <- imputed_df$df
    ls <- imputed_df$ls
    as <- imputed_df$as
...
ENCE_impute <- function(df, missing_idx, ls = NA, as = NA, cluster = NULL, ...){
  ...
  for (column in 1:ncol(df)){
    ...
    imputed_column <- column_impute(covariates, target, missing_values, 
                                    ls[column], as[column], cluster = cluster, ...)
