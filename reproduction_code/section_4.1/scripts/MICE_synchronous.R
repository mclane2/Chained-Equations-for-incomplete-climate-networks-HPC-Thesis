...
## - Parallelism for this file is just for multiple imputation
## - across 
## - Added RMSE tracking
...
MICE_DURR_sync <- function(df, response = "y", m = 10, 
  ...
  if(nclusters > 1){
    ...
    results <- parLapply(cl = cl, df_copies, MICE_get_params_sync, 
    ...
    results <- lapply(df_copies, MICE_get_params_sync,
                     missing_idx = missing_idx,
                     max_cycles = max_impute_cycles, ...)
  }
...
MICE_get_params_sync <- function(df, missing_idx, max_cycles = 16, 
                            hyp_cycles = 2,
  ...
  while((i <= max_cycles) & ((old_rmse/new_rmse > impute_tol) | (i == 1))){
    ...
    # Loop through and update each column (synchronous, but not parallel)
    results <- lapply(1:ncol(df_old), function(column) {
