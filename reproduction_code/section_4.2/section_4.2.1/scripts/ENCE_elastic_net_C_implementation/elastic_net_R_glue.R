...
dyn.load("scripts/ENCE_elastic_net_C_implementation/ENCE_impute.so")

# One synchronous imputation cycle, parallel across stations in C.
# Returns list(df, lambda, alpha).
ence_impute_cycle <- function(df_old, missing_idx, folds, lambda_io, alpha_io, 
  ...
  .Call("ence_impute_cycle_R", df_mat, miss, fld,
        ...
        as.integer(nthreads), lam, alp, PACKAGE = "ENCE_impute")
}